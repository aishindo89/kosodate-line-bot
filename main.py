...
def handle_message(event):
    ...
    context_prompt += "### ステップ3: 回答を生成する際の共通ルール\n"
    context_prompt += "- **【最重要: URLの正確性】** 公式情報リストの情報を使う場合も、ウェブ検索をする場合も、**URLは絶対に創作しないでください。** 不明な場合は正直に「公式サイトURL不明」と記載してください。\n"
    context_prompt += "- **【対話の深化】** もしユーザーの質問が曖昧で追加情報が必要な場合、具体的な質問をしてください。ただし、同じパターンの質問を2回以上連続して使わず、3回目以降は「他に何かお聞きになりたいことはありますか？」のように表現を変えてください。\n"
- **【応答の自然さ】** 役立つ情報が見つからなくても、そのこと自体には言及せず、自然な会話を続けてください。\n"
    context_prompt += "- 最後に、上記のすべてを考慮して、一つの自然でまとまりのある文章として回答を作成してください。\n\n"
    # --- ▲▲▲ ここまでが修正点です ▲▲▲ ---
